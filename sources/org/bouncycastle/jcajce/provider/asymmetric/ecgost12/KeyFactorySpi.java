...
public class KeyFactorySpi extends BaseKeyFactorySpi {
    private boolean isValid(k kVar) {
        if (!kVar.x(do9.g) && !kVar.x(do9.h) && !kVar.x(do9.l) && !kVar.x(do9.m)) {
            return false;
        }
        return true;
    }
    ...
    public PrivateKey engineGeneratePrivate(KeySpec keySpec) throws InvalidKeySpecException {
        if (keySpec instanceof ob3) {
            return new BCECGOST3410_2012PrivateKey((ob3) keySpec);
        }
    ...
    public PublicKey engineGeneratePublic(KeySpec keySpec) throws InvalidKeySpecException {
        if (keySpec instanceof rb3) {
            return new BCECGOST3410_2012PublicKey((rb3) keySpec, ff0.b);
        }
    ...
    public PrivateKey generatePrivate(dk8 dk8Var) throws IOException {
        k t = dk8Var.w().t();
        if (isValid(t)) {
            return new BCECGOST3410_2012PrivateKey(dk8Var);
        }
    ...
    public PublicKey generatePublic(b1b b1bVar) throws IOException {
        k t = b1bVar.t().t();
        if (isValid(t)) {
            return new BCECGOST3410_2012PublicKey(b1bVar);
        }
        throw new IOException("algorithm identifier " + t + " in key not recognised");
    }
}
