...
public class JKSKeyStoreSpi extends KeyStoreSpi {
    ...
    public static final class ErasableByteStream extends ByteArrayInputStream {
        ...
        public void erase() {
            tr.y(((ByteArrayInputStream) this).buf, (byte) 0);
        }
    }

    public JKSKeyStoreSpi(d95 d95Var) {
        this.helper = d95Var;
    }

    private void addPassword(e23 e23Var, char[] cArr) throws IOException {
        for (int i = 0; i < cArr.length; i++) {
            e23Var.update((byte) (cArr[i] >> '\b'));
            e23Var.update((byte) cArr[i]);
        }
        e23Var.update(a0b.e("Mighty Aphrodite"), 0, 16);
    }
    ...
    private ErasableByteStream validateStream(InputStream inputStream, char[] cArr) throws IOException {
        e23 digest = DigestFactory.getDigest("SHA-1");
        byte[] b = aza.b(inputStream);
        if (cArr != null) {
            addPassword(digest, cArr);
            digest.update(b, 0, b.length - digest.getDigestSize());
            int digestSize = digest.getDigestSize();
            byte[] bArr = new byte[digestSize];
            digest.doFinal(bArr, 0);
            byte[] bArr2 = new byte[digestSize];
            System.arraycopy(b, b.length - digestSize, bArr2, 0, digestSize);
            if (tr.u(bArr, bArr2)) {
                return new ErasableByteStream(b, 0, b.length - digestSize);
            }
            tr.y(b, (byte) 0);
            throw new IOException("password incorrect or store tampered with");
        }
        return new ErasableByteStream(b, 0, b.length - digest.getDigestSize());
    }
